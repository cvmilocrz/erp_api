...
export const createUsers = async (req, res) => {
  ...
  try {
    ...
    const client = await getConnection();
    await client.query(queries.users.createUsers, [name, cargo, email, phone, departament, rol, permissions, user_status, hashedPassword]);
    await client.end();
...
export const updateUsers = async (req, res) => {
  ...
  try {
    const hashedPassword = await bcrypt.hash(user_password, 10);
    const client = await getConnection();
    await client.query(queries.users.updateUsers, [name, cargo, email, phone, departament, rol, permissions, user_status, hashedPassword, id]);
    await client.end();
    return res.status(201).json({
